...
contract LeverageRegistry is Ownable {
  ...
  constructor(address _poolProvider, address _curveProvider, address payable _weth, address _arth) {
    ADDR_pool = IPoolAddressesProvider(_poolProvider).getPool();
    ADDR_priceOracle = IPoolAddressesProvider(_poolProvider).getPriceOracle();
  ...
  function getLeverageOf(address _owner) public view returns (address _leverage) {
